...
if [ -z ${CHIA_BRANCH} ]; then
    ...
    if [[ "${arch_name}" == "x86_64" ]]; then
        curl -sLJO https://github.com/Chia-Network/chia-blockchain/releases/download/${CHIA_BRANCH}/chia-blockchain-cli_${CHIA_BRANCH}-1_amd64.deb
        apt-get install ./chia-blockchain-cli*.deb
    else
        curl -sLJO https://github.com/Chia-Network/chia-blockchain/releases/download/${CHIA_BRANCH}/chia-blockchain-cli_${CHIA_BRANCH}-1_arm64.deb
        apt-get install ./chia-blockchain-cli*.deb
    fi
